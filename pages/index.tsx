import type { NextPage } from "next";
import Head from "next/head";
import Card from "../components/Card";
import Layout from "../components/Layout";

const Home: NextPage = () => {
  return (
    <Layout>
      <Head>
        <title>Home</title>
      </Head>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <Card className="bg-stone-100">
          <div className="text-lg font-medium">Card Title</div>
          <div className="text-md">Subtitle</div>
          <div className="grid grid-cols-2 grid-rows-2 gap-4 mt-2 content-center">
            <div className="bg-white w-full h-full shadow shadow-stone-50 rounded-lg p-1">
              <img
                src="https://store.storeimages.cdn-apple.com/4668/as-images.apple.com/is/iphone-13-pro-family-hero?wid=940&hei=1112&fmt=png-alpha&.v=1644969385433"
                alt="product1"
              />
            </div>
            <div className="bg-white w-full h-full shadow shadow-stone-50 rounded-lg p-1">
              <img
                src="https://store.storeimages.cdn-apple.com/4982/as-images.apple.com/is/ipad-pro-12-select-wifi-spacegray-202104_FMT_WHH?wid=940&hei=1112&fmt=p-jpg&qlt=95&.v=1617126635000"
                alt="product2"
              />
            </div>
            <div className="bg-white w-full h-full shadow shadow-stone-50 rounded-lg p-1">
              <img
                src="https://store.storeimages.cdn-apple.com/4982/as-images.apple.com/is/MK2E3?wid=2000&hei=2000&fmt=jpeg&qlt=95&.v=1626468075000"
                alt="product3"
              />
            </div>
            <div className="bg-white w-full h-full shadow shadow-stone-50 rounded-lg p-1">
              <img
                src="https://store.storeimages.cdn-apple.com/4982/as-images.apple.com/is/MQ052?wid=4000&hei=1800&fmt=jpeg&qlt=95&.v=1495129815011"
                alt="product4"
              />
            </div>
          </div>
        </Card>

        <Card>
          <img
            className="mx-6 bg-stone-50"
            alt="Macbook Pro"
            src="data:image/jpeg;base64,"
          />
          <div className="text-orange-600 text-xs">Best Seller</div>
          <div className="font-medium text-md text-slate-800 mt-2">
            2021 Macbook Pro M1 Max, 1TB Unified Memory, 16GB RAM, Space Gray
          </div>
          <div className="text-slate-400 text-xs">Apple</div>
          <div className="mt-3 text-light text-slate-600">$1599.99</div>
        </Card>
      </div>
    </Layout>
  );
};

export default Home;
